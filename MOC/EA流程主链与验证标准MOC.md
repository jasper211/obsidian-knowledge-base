...
- [L3编码注册标准v2.0](/Users/a112233/Desktop/流程架构项目_jasper/03_发布成果-交付物/治理规范/[STD]_L3编码注册标准_v2.0.md) 声明为03发布规范，指定现行注册表，并将历史迁移与现行注册分开；不能用历史映射直接更新当前编码。
- [VS-L3现行关系表](/Users/a112233/Desktop/流程架构项目_jasper/03_发布成果-交付物/权威数据/[DATA]_VS-L3_现行关系_v1.0.csv) 是该标准指定的注册入口。已定向核查L3-COM：分类为NON_VS_CONSUMED，被VS-1、VS-2消费，不是这两个价值流的Owner；不据此推导上下游流程或人员任命。
- [8/26 P5发布授权记录](/Users/a112233/Desktop/流程架构项目_jasper/00_治理与元模型/变更记录/变更记录_2026-08-26_Mark_ACC004_P5集中发布授权.md) 记录有限范围发布授权，PUB-08—10仍HOLD，PG操作未授权；不能扩大成整项目验收。本轮未核发布哈希、回归回执或数据库实况。

调用顺序：编码/归属问题先读上述源表；流程说明再读下方卡片，并保留其草稿、来源和未验证边界。标准所述75码及覆盖率是源文件报告值，本轮没有重新验证全表覆盖。

### 局部草稿流程参考

- [[EA流程架构项目/SOP目标|EQ机构侧方案设计草稿]]：建档 → 拆解 → Gate → 评审冻结，并可转入 SOB 上架流程。
- [[EA流程架构项目/L3-SOB流程目标|SOB服务项寻源与上架草稿]]：路径分类 → 寻源 → 设计 → Gate → 上架条件。

以上来源的 v0.1 SOP 已核实声明为草稿，仅供规划与分析，不是已批准执行标准或业务完成证据。审批 Gate、外发 G1-G7 和挂数/落地/追溯三重 Gate 应按各自适用流程使用，不能互相替代。旧 [[EA流程架构项目/SOP管理|SOP管理]] 已改为历史分流页。
...
本卡自述为draft、未经Terresa/Mark确认；`completeness: complete`只描述卡片填写，不表示业务审批、18个L4或KPI均已复核。本轮仅核编码及VS消费归属，未验证卡片全部业务断言。
