...
│   └── README.md
├── _待审_GitHub发现/    ← GitHub自动发现候选区（AI可写，人工确认后转正进raw/，
│                          详见该目录下 README.md）
│   ├── README.md
│   └── _已处理/          ← 已转正/已拒绝的候选归档处
└── wiki/                ← 知识库层（AI 摄入后维护）
...
- `wiki/` 摄入（Ingest）操作仍是人工/对话驱动，不是自动化脚本
- **`raw/` 只能人工放入资料，AI 不主动抓取，这条规则不变**（2026-07-25起有了一个例外补充，见下条，不是这条规则本身被打破）
- **AI可写入`_待审_GitHub发现/`**（2026-07-25新增，`方法论转正Agent/github_discovery.py`每周扫描精选GitHub清单产出候选）——但这不代表AI获得了`raw/`的写权限，候选必须经人工确认（`promote_candidate.py --promote`或纯手动）才能进入`raw/`，`_待审_GitHub发现/`和`raw/`是两个权限完全不同的目录，不要混淆
