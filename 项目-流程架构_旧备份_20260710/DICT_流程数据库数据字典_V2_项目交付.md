...
创建日期: 2026-04-22
最后更新: 2026-05-29
更新说明: ①dim_org 粒度修正为岗位族级（8行）；②dim_time 标注当前0行待生成；③dim_kpi 更新为32企业KPI已上线；④dim_deliverable 行数更新为400并说明L3/L4演进中；⑤fact_card 字段取值来源同步实际状态
补 frontmatter 日期: 2026-05-22
补 frontmatter agent: Jasper（W4 P0-8 收尾）
源自 01 层: M-01_方法论与标准/VS-CSV列结构规范_V1.md + M-02_架构全景/L3-definition-schema.yaml + L4-Agent化评估框架.md + M-04_项目工作区/流程数据库星型模型设计
关联交付清单批次: '002'
关联映射表行: §2.1 第6行
维护人: Terresa（维护）/ Carrie（技术实施）/ Jasper
...
| FC-009 | kpi_key | 关联 DIM_KPI 的代理键 | 企业KPI 已上线（32 行），但 `fact_card.kpi_key` 关联规则待 KPI穿透矩阵完成后启用 ⚠️ | INT, FK → DIM_KPI(kpi_key), NULLABLE | Phase 1/2 允许为 NULL；DIM_KPI 32 企业 KPI 已存在（5-24 导入），关联规则 Week 6+ 定稿后逐步填入 |
| FC-010 | deliverable_key | 关联 DIM_DELIVERABLE 的代理键 | 按 l4_code 查询 DIM_DELIVERABLE 自动回填 ⚠️ | INT, FK → DIM_DELIVERABLE(deliverable_key), NULLABLE | 当前 400 行，425 L4 中 25 个暂无对应交付物（L3/L4 演进中）；缺失时允许 NULL；L4 定义稳定后补录 |
...
> 粒度：岗位族级（当前 8 行，按 7 族+职能划分；同族内多岗位多人尚未拆分，GAP-01 完成后逐步细化）
> 注：`seed_dim_org.sql`（2026-04-27）插入 8 行，C 族为复合族（用一个记录代表整体），executor_id 均为 NULL（HR 基线数据尚未回传）
...
> 脚本生成，覆盖 2024-01-01 至 2027-12-31（约 1461 行）
> 无外部数据依赖，当前需由 Carrie 执行生成（2026-05-29）
> 注：`create_process_schema.sql` 仅定义表结构，无 INSERT 脚本；当前 DB 实测 0 行
...
> 数据来源：企业KPI 体系（32 项，KPI_01~KPI_51）已于 2026-05-24 由 Carrie 批量导入（`dim_kpi_insert_enterprise.sql`）；旧岗位 KPI 已删除
> 32 企业 KPI 含 l1_code / l3_codes / formula_numerator / formula_denominator 字段（2026-05-23 W8-C 补全，Mark 5-23 verify）
> KPI穿透矩阵尚在制作中，`fact_card.kpi_key` 关联规则 Phase 2（Week 6+）启用
> 注：字典 V1.0 描述"Phase 2 才有"已过时，V2.0 修正为"32 企业 KPI 已上线，岗位 KPI 待穿透矩阵完成后补充"
...
> 数据来源：`L4-Agent化清单.csv` 第4列 + 后续批次补充，当前共 **400 行**（2026-05-28 实测 `dim_deliverable_字典匹配结果.csv`）
> 依据 L4 唯一物理交付物原则（CLAUDE.md 5.0节）：一个 L4 = 一个物理交付物 = DIM_DELIVERABLE 一行
> **注**：当前 L3/L4 定义仍在演进中（M2 裁定持续迭代），`dim_process` 425 L4 中部分新增/合并/重构的 L4 尚未同步到 `dim_deliverable`，缺失 25 个 L4 的交付物记录是正常过渡状态，不影响 Phase 1 灌数（FC-010 `deliverable_key` 已标 NULLABLE）
...
| 项目 | 说明 |
|------|------|
| **当前版本** | V2.0（2026-05-29）|
| **上次版本** | V1.0（2026-04-22）→ V2.0 修正：dim_org 粒度、dim_time 状态、dim_kpi 已上线说明、dim_deliverable 行数与演进说明 |
| **下次更新触发条件** | ①新增维度表字段；②枚举值变更（需 Mark 裁定）；③validate_kb.py 校验规则扩展；④KPI穿透矩阵完成（`fact_card.kpi_key` 关联规则定稿）；⑤L3/L4 定义稳定后 `dim_deliverable` 补录缺失 25 个 L4|
